...
import { MapPin, DollarSign, Award, Building, ExternalLink, Filter, Navigation } from 'lucide-react';
import API from '../services/api';

// Dynamically import Leaflet components for React 18 compatibility
...
export default function CollegesPage() {
  ...
  // Geo & Filter State
  const [userLocation, setUserLocation] = useState({ lat: 19.0760, lng: 72.8777, label: 'Mumbai, MH' });
  ...
  useEffect(() => {
    ...
    fetchColleges();
  }, [userLocation, radiusKm, maxBudget, typeFilter]);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-6">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 text-xs font-semibold mb-2">
            ...
            <span>Interactive Leaflet Geo-Discovery Map</span>
          </div>
          <h1 className="text-3xl font-extrabold text-white">College Geo-Discovery & Matching</h1>
          <p className="text-xs text-slate-400">
            Locate top matched colleges nearby based on geographic radius, tuition budget, and accreditation.
          </p>
        </div>

        <div className="flex items-center gap-2">
          <span className="glass-pill text-emerald-400">📍 Center: {userLocation.label}</span>
        </div>
      </div>
      ...
      {/* Leaflet Map & List Split View */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        
        {/* Map View (2 Cols) */}
        <div className="lg:col-span-2 glass-card p-2 h-[480px] rounded-2xl overflow-hidden relative">
          <MapContainer
              ...
              <Marker key={c.id} position={[c.latitude, c.longitude]}>
                <Popup>
                  <div className="p-1 space-y-1 text-slate-900">
                    ...
                    <span className="text-[11px] block text-slate-600">📍 {c.city}, {c.state} ({c.distanceKm} km away)</span>
        ...
        {/* Colleges List Sidebar (1 Col) */}
        <div className="space-y-4 max-h-[480px] overflow-y-auto pr-1">
          <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">
            Matched Colleges ({colleges.length})
          </h3>
          ...
          {loading ? (
            ...
            <div className="p-6 text-center text-xs text-slate-400 glass-card">
              No colleges match your radius & budget criteria. Try expanding the sliders.
            </div>
          ) : (
            colleges.map((c) => (
              <div key={c.id} className="glass-card p-4 space-y-2 hover:border-blue-500/40">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-blue-500/20 text-blue-300 border border-blue-500/30 shrink-0">
                    {c.distanceKm !== null ? `${c.distanceKm} km` : 'Near'}
                  </span>
                </div>
                ...
                {c.website && (
                  ...
                  >
                    College Portal <ExternalLink className="w-3 h-3" />
                  </a>
                )}
              </div>
            ))
