...
// PATCH /api/roadmap/:userId/milestone/:id
router.patch('/:userId/milestone/:id', authenticateToken, async (req, res) => {
  try {
    const userId = req.params.userId || req.user.id;
    const milestoneId = req.params.id;
    const { status } = req.body; // 'completed' | 'in-progress' | 'pending'
...
router.post('/generate', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(201).json({
      message: `Roadmap generated for ${targetCareer}`,
      roadmap: {
        id: roadmapRecord.id,
        targetCareer,
        milestones,
